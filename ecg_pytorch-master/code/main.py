...
from data_process import ECGDataset
...
#用于测试加载模型
def val(args):
    list_threhold =np.random.rand(50)
    model = getattr(models, config.model_name)()
    model = model.to(device)
...
def test(args):
    ...
    from lable_process import name2index
    ...
    model.eval()
    sub_file = '%s/result.txt' % (config.sub_dir)
